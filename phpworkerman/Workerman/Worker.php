...
class Worker
{
    ...
    {
        //检查是否为命令行，定义操作系统self::$_OS
        static::checkSapiEnv();


        //定义开始文件Start file(static::$_startFile)
        //定义pid_file(static::$pidFile)和log_file(static::$logFile)
        //定义status(static::$_status)和status_file(static::$_statisticsFile)
        //定义进程标题(Process title)
        //初始化id_map(static::$_idMap)
        //初始化timer
        static::init();


        //锁住开始文件(static::$_startFile)
        static::lock();


        //php http_test.php[0] start[1]
        //解析命令行参数必须为'start'/'stop'/'restart'/'reload'/'status'/'connections',
        //第二个参数(-d)配合第一个参数为start时候使用，后台进程模式
        static::parseCommand();


        //如果是后台进程模式启动则失去当前终端的控制权
        static::daemonize();



        //其实主要是调用$worker->listen()
        //这个函数主要作用是先检查$this->_mainSocket是否存在，如果不存在则创建stream_socket_server
        //listen()中调用resumeAccept()函数
        //而这个函数在没有注册static::$globalEvent时是没有效果的。
        static::initWorkers();
        ...
        //获取主进程号赋值static::$_masterPid
        //并保存在pid的文件static::$pidFile中
        static::saveMasterPid();


        //解锁开始文件(static::$_startFile)
        static::unlock();
        ...
        //主进程开始fork出设置的n个子进程
        //循环调用forkOneWorkerForLinux()函数，$worker(new Worker())作为参数传递
        //while (count(static::$_pidMap[$worker->workerId]) < $worker->count) {
        //      static::forkOneWorkerForLinux($worker);
        //}

        //forkOneWorkerForLinux()函数
        //在主进程中会将static::$_pidMap和static::$_idMap都填上对应的子进程号
        //主进程fork子进程成功以后会跳到下一步static::resetStd();

        //所以这主要分析子进程的动作。
        //1、如果是后台运行模式，则子进程的输出也要重定向static::resetStd();
        //2、将子进程空间的pidMap清空，static::$_pidMap  = array();
        //3、移除其他监听事件
        //4、子进程空间调用$worker->run();

        //子进程的动作大部分是在这个函数进行的。所以再分析一下这个函数。
        //1、将子进程的状态更新static::$_status = static::STATUS_RUNNING;
        //2、创建子进程空间的全局事件static::$globalEvent = new \Workerman\Events\Select()
        //这里暂时先研究select事件模型，注意new的时候会调用构造函数，构造函数进行如下步骤
        //创建一个管道并将其放入read的集合中以读取描述符，以避免空轮询
        //
        //3、调用$this->resumeAccept();此时的static::$globalEvent已经存在
        //static::$globalEvent->add($this->_mainSocket, EventInterface::EV_READ, array($this, 'acceptConnection'));
        //意思是让子进程监听客户端的连接。
        //$this->_allEvents[$fd_key][$flag] = array($func, $fd) = array(array($this,'acceptConnection'),$fd);
        //将main_socket放入read的集合中$this->_readFds[$fd_key] = $fd;
        //此时read的集合中已经有两个元素[0]=创建的那个通道，[$fd_key] = $fd。

        //4、调用事件的loop()函数循环监听static::$globalEvent->loop();
        //循环...

        //如果连接的话：
        //先调用acceptConnection()函数，并将main_socket作为参数传入
        ...
        //如果是请求的话
        //先调用TcpConnection 的 baseRead，并将$new_socket作为参数传入
        //读取请求参数，之后调用onMessage()，如果该函数有$connection->send('hello world');则会将发送的信息加入全局事件中
