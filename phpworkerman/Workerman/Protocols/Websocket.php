...
class Websocket implements \Workerman\Protocols\ProtocolInterface
{
    ...
    public static function input($buffer, ConnectionInterface $connection)
    {
        ...
        if ($connection->websocketCurrentFrameLength) {
            ...
            $firstbyte    = ord($buffer[0]);//第一个字节
            $secondbyte   = ord($buffer[1]);//第二个字节
            /**
             * 因为七个bit位所能够表达的最大长度为127，所以将其分为一下三种情况
             *
             * 1、如果其值在0-125之间，则是payload的真实长度
             * 2、如果其值为126，则后面的两个字节(16bit位)形成的无符号整型数值是payload的真实长度
             * 3、如果其值是127，则后面的八个字节(64bit位)形成的无符号整型数值是payload的真实长度
             */
            $data_len     = $secondbyte & 127;//根据第二个字节的后7位(127=1111111)算数据包的长度
            $is_fin_frame = $firstbyte >> 7;//第一个字节右移7位，取得第一字节的第一位，表示是否为最后一个数据帧
            $masked       = $secondbyte >> 7;//第二个字节右移7位，取得第二字节的第一位，表示是否掩码（客户端向服务端发送数据必须掩码）

            if (!$masked) {//如果客户端向服务端没有掩码，则关闭连接
                Worker::safeEcho("frame not masked so close the connection\n");
                $connection->close();
                return 0;
            }

            $opcode       = $firstbyte & 0xf;//第一个字节与1111或运算，取出第一个字节的后四位，从而获取操作码
            switch ($opcode) {
                case 0x0://表示延续帧
                    break;
                // Blob type.
            ...
            $head_len = 6;//头长度
            if ($data_len === 126) {
                $head_len = 8;//如果数据长度位126的话，后面会多两个字节表示数据长度，所以+2
                ...
                if ($data_len === 127) {
                    $head_len = 14;//如果数据长度位127的话，后面会多八个字节表示数据长度，所以+7
                    if ($head_len > $recv_len) {
                        return 0;
                    }
